...
function toCalculatorInput(draft: RecreationalPlanDraft): RecreationalAirDiveInput {
  return {
    unitSystem: draft.unitSystem,
    depth: draft.depth,
    bottomTime: draft.bottomTime,
    gas: draft.gas,
    repetitiveDive: draft.isRepetitive
      ? {
          surfaceIntervalMinutes: draft.surfaceIntervalMinutes,
          secondDiveDepth: draft.secondDiveDepth,
          secondDiveBottomTime: draft.secondDiveBottomTime
        }
      : undefined
  };
}
...
export function App() {
  ...
  const updateDraft = (patch: Partial<RecreationalPlanDraft>) =>
    setDraft((currentDraft) => ({ ...currentDraft, ...patch }));

  const startNewPlan = () => {
    setDraft(INITIAL_RECREATIONAL_DRAFT);
    setResult(null);
    setCurrentStep("plan");
  };

  const startDemoPlan = () => {
    const demoDraft: RecreationalPlanDraft = {
      unitSystem: "metric",
      depth: 18,
      bottomTime: 20,
      gas: "air",
    ...
    setDraft(demoDraft);
    setResult(calculateRecreationalAirDive(toCalculatorInput(demoDraft)));
    setCurrentStep("result");
  };

  const calculatePlan = () => {
    setResult(calculateRecreationalAirDive(toCalculatorInput(draft)));
    setCurrentStep("result");
  };
